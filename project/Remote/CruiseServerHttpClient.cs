        ...
        : CruiseServerClientBase
    {
        ...
        public override ProjectStatus[] GetProjectStatus()
        {
            try
            {
                ...
                var uri = new Uri(url);
                webFunctions.SetCredentials(client, uri);
                string response;
                try
                {
                ...
                catch (WebException error)
                {
                    if (error.Message.Contains("(403) Forbidden"))
                    {
                        ...
                        var forceAuthorizationWebFunctions = new ForceAuthorizationWebFunctions();
                        forceAuthorizationWebFunctions.SetCredentials(client, uri);
                        response = client.DownloadString(url);
                    }
                    else
                    {
        ...
        public override CruiseServerSnapshot GetCruiseServerSnapshot()
        {
            try
            {
                ...
                try
                {
                    ...
                    webFunctions.SetCredentials(client, new Uri(url));
                    response = client.DownloadString(url);
                }
                catch (Exception)
                {
